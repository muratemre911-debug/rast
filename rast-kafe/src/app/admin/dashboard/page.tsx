  ...
  return (
    <div className="min-h-screen bg-zinc-950 pb-20">
      <header className="sticky top-0 z-50 bg-zinc-900 border-b border-zinc-800">
        ...
        <div className="flex px-4 pb-3 gap-2 flex-wrap justify-center">
          <button
            onClick={() => setActiveTab('products')}
            className={`px-3 py-2 rounded-lg text-sm font-medium flex items-center justify-center gap-1.5 min-w-[80px] ${activeTab === 'products' ? 'bg-[#B33F2E] text-white' : 'bg-zinc-800 text-zinc-400'}`}
          >
            <Package className="w-4 h-4" /> {t.products}
          </button>
          <button
            onClick={() => setActiveTab('categories')}
            className={`px-3 py-2 rounded-lg text-sm font-medium flex items-center justify-center gap-1.5 min-w-[80px] ${activeTab === 'categories' ? 'bg-[#B33F2E] text-white' : 'bg-zinc-800 text-zinc-400'}`}
          >
            <Store className="w-4 h-4" /> {t.categories}
          </button>
          <button
            onClick={() => setActiveTab('settings')}
            className={`px-3 py-2 rounded-lg text-sm font-medium flex items-center justify-center gap-1.5 min-w-[80px] ${activeTab === 'settings' ? 'bg-[#B33F2E] text-white' : 'bg-zinc-800 text-zinc-400'}`}
          >
            <Settings className="w-4 h-4" /> {t.settings}
          </button>
          <button
            onClick={() => setActiveTab('analytics')}
            className={`px-3 py-2 rounded-lg text-sm font-medium flex items-center justify-center gap-1.5 min-w-[80px] ${activeTab === 'analytics' ? 'bg-[#B33F2E] text-white' : 'bg-zinc-800 text-zinc-400'}`}
          >
